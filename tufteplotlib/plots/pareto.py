...
def pareto_chart(categories, values, *,
    ...
    ax.bar(x_pos, values, color=color, alpha=alpha, width=bar_width, bottom=0)
    ...
    ax2 = ax.twinx()
    ax2.plot(x_pos, cumulative_pct, color='black', linewidth=line_width, alpha=0.8, zorder=3)
    ax2.scatter(x_pos, cumulative_pct, color='black', s=dot_size,
                edgecolor='white', linewidth=0.8, zorder=4)

    # Percentage labels above dots
    for i, pct in enumerate(cumulative_pct):
        ax2.text(x_pos[i], pct + 1, f"{pct:.1f}%", ha='center', va='bottom', fontsize=9)
